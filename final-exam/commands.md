...
cd ~/OS-SE-20240063/final-exam/partC_scripting/scripts
chmod +x greeter
./greeter
mkdir -p ~/bin
cp greeter ~/bin/
...
echo "$PATH"
which greeter
cd ~/OS-SE-20240063/final-exam/partC_scripting
echo "=== PATH Report ===" > path_report.txt
echo "PATH:" >> path_report.txt
echo "$PATH" >> path_report.txt
echo "" >> path_report.txt
echo "Resolved location of greeter:" >> path_report.txt
which greeter >> path_report.txt
cd scripts
chmod +x collector
mkdir -p data1 data2 data3
echo "Sales notes for Q1" > data1/notes.txt
echo "Confidential report" > data1/secret.txt
chmod 000 data1/secret.txt
echo "Status report from data2" > data2/report.txt
./collector
