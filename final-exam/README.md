...
The shell only searches directories listed in `$PATH` when resolving a bare
command name. A script sitting in an arbitrary folder (like
`final-exam/partC_scripting/scripts/`) is not found unless that folder is
added to `$PATH`, or invoked explicitly with `./greeter` or a full path.
In my case `~/bin` was already present in `$PATH` from earlier coursework
setup, so once I copied `greeter` into `~/bin/`, it resolved immediately
by name without further changes.
